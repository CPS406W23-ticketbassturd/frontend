...
export function getSearch() {
    ...
    useEffect(() => {
        ...
        const fetchData = () => {
            fetch(`http://localhost:8000/api/search/venues/${localStorage.getItem('query')}`, {method: 'GET', headers: {'Content-Type': 'application/json', 'Alow-Control-Allow-Origin': '*'}})
                .then(data => data.json())
                    ...
                    <Grid2 item xs={12} sm={6} md={4} lg={3} xl={2}>
                        <Card sx={{m: 1, p: 1, border: '1px solid grey', borderRadius: '25px', boxShadow: 2, width: '40em'}}>
                            <Stack alignItems={'left'} justifyContent={'left'} sx={{pt: 1, backgroundColor: 'white', p: 3, m: 1, border: '1px solid grey', borderRadius: '25px', boxShadow: 2}}>
                                ...
                                <Typography sx={{m: 1, color: 'black'}} variant={'body1'}>{`Address: ${item.address}`}</Typography>
                                <Typography sx={{m: 1, color: 'black'}} variant={'body1'}>{`Max Capacity: ${item.max_capacity}`}</Typography>
                                <Link href={`/venue?id=${item.venue_id}`}>
...
export default function VenueSearch() {
    return (
        <>
            <Head>
                <title>Search Venue | Ticketbass-Turd</title>
            ...
            <main>
                ...
                <Stack id='results' alignItems={'center'} spacing={2} sx={{mt: 5}}>
                    <Typography variant={'h5'}>Venue Results for:</Typography>
